...
export class ConverterService {
  ...
  private async createImage(page: Page, fileName: string, isPreview: boolean): Promise<any> {
    ...
    if (this.isRemoteUploadEnabled) {
      const data = await this.cloudinaryService.uploadFile(imageBuffer as Buffer, convertType, uploadFileName, false)
      return { resourceUrl: data?.secure_url }
    } else {
      await this.fileService.folderExistenceCheck(`${this.attachmentFolder}/${convertType}`)
      await this.fileService.createFile(this.getPath(this.attachmentFolder, uploadFileName, convertType), imageBuffer as Buffer)
  ...
  async initialURLHealhCheck(url: string, convertType?: ConvertTypes | ImageTypes | AudioTypes): Promise<boolean> {
